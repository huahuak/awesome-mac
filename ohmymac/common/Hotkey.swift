...
class Hotkey {
    ...
    // MARK: delayTriggerFunc
    private var delayTime: Double = 0.2 // sec
    private var delayTriggerCondition: Bool = false
    func delayTrigger(modifier: NSEvent.ModifierFlags, handler: @escaping Fn) {
        let fn: (NSEvent) -> Void = { [self] event in
            if !delayTriggerCondition &&
                event.type == .flagsChanged &&
                flagsEq(eventFlags: event.modifierFlags, flag: modifier) {
                delayTriggerCondition = true
                triggerTime = event.timestamp
                return
            }
            if delayTriggerCondition && event.type == .keyDown {
                delayTriggerCondition = false
                return
            }
            if delayTriggerCondition &&
                event.type == .flagsChanged &&
                event.timestamp - triggerTime < delayTime {
                delayTriggerCondition = false
                triggerTime = 0
                handler()
            }
        }
        if let monitor = NSEvent.addGlobalMonitorForEvents(matching: [.keyDown, .flagsChanged], handler: fn) {
            deInitFunc.append {
                NSEvent.removeMonitor(monitor)
            }
        }
